...
export default function EditEventPage() {
    ...
    const fetchEvent = async () => {
      try {
        ...
        if (snap.exists()) {
          ...
          setTimeFrom(data.timeFrom ? new Date(`1970-01-01T${data.timeFrom}`) : new Date());
          setTimeTo(data.timeTo ? new Date(`1970-01-01T${data.timeTo}`) : new Date());
          setLocation(data.location || "");
          setProvince(data.province || "");
          setMaxAttendees(data.maxAttendees?.toString() || "");
  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={{ paddingTop: 25, paddingBottom: 70 }}
      showsVerticalScrollIndicator={false}
    >
      <View style={[styles.card, { marginBottom: 10 }]}>
        <Text style={styles.sectionTitle}>✏️ Edit Event</Text>
      ...
      <View style={styles.card}>
        ...
        {uploadProgress && (
          <Text style={{ textAlign: "center", marginBottom: 8 }}>{uploadProgress}</Text>
        )}
        {imageUrl ? (
          <Image
            source={{ uri: imageUrl }}
            style={styles.previewImage}
            onError={() => Alert.alert("Invalid URL", "Could not load image from link.")}
          />
        ) : null}
      </View>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.toggleRow}>
          <Pressable
            style={[styles.toggleButton, isFree && styles.activeToggle]}
...
const styles = StyleSheet.create({
  ...
  saveButton: { backgroundColor: COLORS.accent, paddingVertical: 16, borderRadius: 10, alignItems: "center", marginBottom: 60 },
